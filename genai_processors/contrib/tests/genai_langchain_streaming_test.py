...
class GenAILangChainProcessorTest(parameterized.TestCase):
    ...
    def test_streaming_sync(self):
        ...
        parts = [
            content_api.ProcessorPart("Question?", mimetype="text/plain", role="user")
        ]
        output = processor.apply_sync(proc, parts)
    ...
    def test_memory_accumulates(self):
        ...
        # First call
        processor.apply_sync(proc, [
            content_api.ProcessorPart("First turn", role="user")
        ])
        # Second call
        processor.apply_sync(proc, [
            content_api.ProcessorPart("Second turn", role="user")
        ])
